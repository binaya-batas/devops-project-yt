...
- <b>Get NVD API Key for OWASP Dependency Check:</b>
  - Go to [NVD API Key Request](https://nvd.nist.gov/developers/request-an-api-key) and request an API key
  - Once you receive the API key, go to <mark>Manage Jenkins --> Credentials --> System --> Global credentials</mark>
  - Click on <mark>Add Credentials</mark> and select <mark>Kind: Secret text</mark>
  - Set the <mark>ID</mark> as `NVD_API_KEY` and paste your NVD API key in the <mark>Secret</mark> field
  - This API key is required for OWASP Dependency Check to download the vulnerability database from NVD
